...
def upload_img(img_bytes):
    ...
    # Try 2: postimages.org (free, no key needed)
    try:
        buf.seek(0)
        files = {'file': ('image.jpg', buf.getvalue(), 'image/jpeg')}
        r = requests.post("https://postimages.org/json/rr", files=files, timeout=30)
        if r.ok and r.json().get("url"):
            return r.json()["url"]
    except:
        pass
    
    # Try 3: litterbox (temp hosting)
    try:
        buf.seek(0)
        files = {'fileToUpload': ('image.jpg', buf.getvalue(), 'image/jpeg')}
        data = {'reqtype': 'fileupload', 'time': '72h'}
        r = requests.post("https://litterbox.catbox.moe/resources/internals/api.php", files=files, data=data, timeout=30)
...
with t1:
    if not st.session_state.order:
        ...
        if all_imgs:
            st.markdown("---")
            st.write(f"**📋 {len(all_imgs)} Photos**")
            cols = st.columns(4)
            for i, img in enumerate(all_imgs): 
                cols[i%4].image(img, caption=i+1, use_container_width=True)
            
            c1, c2 = st.columns(2)
            if c1.button("🗑️ Clear All", use_container_width=True):
                st.session_state.imgs = []
                st.rerun()
            ...
            if c2.button(f"💾 SAVE {st.session_state.order}", type="primary", use_container_width=True):
                ...
                if urls:
                    ...
                    st.error("❌ Upload failed! Internet check karo.")
